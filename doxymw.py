...
def main():
    ...
    gen = pagegenerators.CategorizedPageGenerator(options["mediaWiki_docsCategory"], recurse=True)
    ...
    for pageData in wikiPages:
        ...
        if options["mediaWiki_setupTranscripts"]:
            ...
            if "mediaWiki_transcriptionPrefix" in options and options["mediaWiki_transcriptionPrefix"] != "":
                transPrefix = options["mediaWiki_transcriptionPrefix"] + " "
                
            transCategory = ""
            if "mediaWiki_transcriptionCategory" in options and options["mediaWiki_transcriptionCategory"] != "":
                transCategory = "[[Category:" + options["mediaWiki_transcriptionCategory"] + "]]"    
            ...
            for page in gen:
                ...
                if not page.exists() or page.isRedirectPage():
                    text = "#REDIRECT [[" + mwTitle + "]]\n" + transCategory
                    
                #If the page does exist, make it a transcription page
                #This ONLY happens if the page is in our category
                    ...
                    if hasCat:
                        page.get()
                        pos = text.find("{{" + mwTitle + "}}")
                        if pos == -1:
                            #Append the transcription to the page, but only if it isn't already there
                            text = page.text + "\n{{" + mwTitle + "}}"
                ...
                if text != "":
                    try:
                        ...
                        print("WARNING: Could not create transcriptions page.")
                        continue
